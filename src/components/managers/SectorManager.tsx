...
export default function SectorManager() {
  ...
  const columns: ColumnDef<Sector>[] = [
    { accessorKey: 'economicSector', header: 'Economic Sector', cell: ({ row }) => <span className="font-medium text-white">{row.original.economicSector}</span> },
    { accessorKey: 'macroPd', header: 'Macro PD', cell: ({ row }) => <span className="text-teal-400 font-mono">{row.original.macroPd.toFixed(4)}</span> },
    ...(isAdmin ? [{
  ...
  return (
    <Card className="glass-card border-white/5">
      <CardHeader className="flex flex-row items-center justify-between">
        <CardTitle className="text-white flex items-center gap-2"><Building2 className="h-5 w-5 text-teal-400" />Sectors</CardTitle>
        {isAdmin && (
          <Button onClick={openCreate} className="bg-gradient-to-r from-teal-500 to-teal-600 text-white shadow-lg shadow-teal-500/20">
      ...
      <FormModal open={modalOpen} onOpenChange={setModalOpen} title={editing ? 'Edit Sector' : 'Create Sector'}>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="space-y-2">
            <Label className="text-gray-300">Economic Sector</Label>
            <Input {...register('economicSector')} className="bg-white/5 border-white/10 text-white" />
            {errors.economicSector && <p className="text-xs text-red-400">{errors.economicSector.message}</p>}
          </div>
          <div className="space-y-2">
            <Label className="text-gray-300">Macro PD</Label>
            <Input type="number" step="0.0001" {...register('macroPd')} className="bg-white/5 border-white/10 text-white" />
