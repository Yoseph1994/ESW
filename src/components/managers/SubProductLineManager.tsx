...
export default function SubProductLineManager() {
  ...
  const columns: ColumnDef<SubProductLine>[] = [
    ...
    { accessorKey: 'productGroupId', header: 'Product Group', cell: ({ row }) => <span className="text-teal-400">{getParentName(row.original.productGroupId)}</span> },
    ...(isAdmin ? [{
  ...
  return (
    <Card className="glass-card border-white/5">
      <CardHeader className="flex flex-row items-center justify-between">
        <CardTitle className="text-white flex items-center gap-2"><GitBranch className="h-5 w-5 text-teal-400" />Sub Product Lines</CardTitle>
      ...
      <FormModal open={modalOpen} onOpenChange={setModalOpen} title={editing ? 'Edit Sub Product Line' : 'Create Sub Product Line'}>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label className="text-gray-300">Product Group</Label>
            <Select value={watch('productGroupId')} onValueChange={(v) => setValue('productGroupId', v)}>
              <SelectTrigger className="bg-white/5 border-white/10 text-white"><SelectValue placeholder="Select product group" /></SelectTrigger>
