...
export default function Login() {
  ...
  const onSubmit = async (data: LoginForm) => {
    ...
    try {
      ...
      // For demo: allow login with admin/password or crm_manager/password123
      if (data.username === 'admin' && data.password === 'password') {
        ...
        login('demo-token-crm-456', 'Abebe Kebede', 'CRM MANAGER');
        toast.success('Welcome back!', {
        ...
        <Card className="glass-card border-white/10 shadow-2xl shadow-black/20">
          ...
          <CardContent className="px-8 pb-8 pt-4">
            ...
            <p className="text-center text-xs text-gray-500 mt-6">
              Demo: <span className="text-teal-400/80">admin</span> /{' '}
              <span className="text-teal-400/80">password</span> or{' '}
              <span className="text-teal-400/80">crm_manager</span> /{' '}
              <span className="text-teal-400/80">password123</span>
            </p>
          </CardContent>
        </Card>
      </motion.div>
